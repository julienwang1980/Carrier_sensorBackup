...
float pred_Tdis_old(float p_suc, float t_suc, float p_dis, float compSpeed)
{
	float volume_flow_rate, power;
	float z_fw, coe_a, coe_b, coe_c;
	float ts_suc;	//ts:temperature of saturation suction gas
...
*/
//-------------------------------------------------------------------------------------------------
float pred_Pdis_curr(float P_suc, float I_test, float compSpeed)
{
	...
	for (size_t i = 0; i < 20; i++)
	{
		...
		I = cal_current(Pd_int, P_suc, compSpeed);
		printf("current: %f\n.", I);
...
void sensor_pre_test(void)
{
	...
	while ((dirp = readdir(dp)) != NULL)
	{
		...
		if (strstr(dirp->d_name, "tdis.csv") != NULL)
		{
			...
			t_dis_old = pred_Tdis_old(Ps, ST, Pd, CompSpeed);
			...
			while (fgets(row, 512, fpr) != NULL)
			{
				...
				t_dis_old = pred_Tdis_old(Ps, ST, Pd, CompSpeed);
				t_dis = pred_Tdis(Ps, ST, Pd, CompSpeed, tau, 2.0);
				/* write the data to file */
